...
const TestimonialsList = ({ list }) => (
  <div className="flex gap-8 shrink-0">
    {list.map((testimonial, index) => (
      <div 
        key={index} 
        className="glass-card rounded-2xl p-8 w-[380px] shrink-0 whitespace-normal flex flex-col justify-between hover:border-blue-400/50 dark:hover:border-blue-500/30 transition-colors duration-300"
      >
        <div>
          ...
          <p className="text-slate-700 dark:text-slate-300 text-lg mb-8 italic leading-relaxed">
            "{testimonial.content}"
          </p>
        </div>
        <div className="flex items-center gap-4 mt-auto">
          ...
          />
          <div>
            <h4 className="text-slate-900 dark:text-white font-bold">{testimonial.name}</h4>
            <p className="text-sm text-slate-600 dark:text-slate-500">{testimonial.role}</p>
          </div>
        </div>
      </div>
    ))}
  ...
  return (
    <section id="testimonials" className="py-24 relative overflow-hidden bg-slate-50 dark:bg-slate-900/50">
      ...
      <motion.div 
        ...
        className="relative flex overflow-x-hidden w-full py-4 mask-gradient"
      >
        {/* Soft edge gradients */}
        <div className="absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-slate-50/90 dark:from-navy-900/90 to-transparent z-10 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l from-slate-50/90 dark:from-navy-900/90 to-transparent z-10 pointer-events-none" />
